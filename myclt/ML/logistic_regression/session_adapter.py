...
Implements extract() and restore() to convert between AppState and SessionData.
"""
...
class LogisticRegressionSessionAdapter(SessionAdapter):
    """Adapter for Logistic Regression session serialization"""
    ...
    def restore(self, session_data: SessionData, arrays: Dict[str, np.ndarray], app_state: Any) -> None:
        ...
        app_state.learning_rate = hyperparams.get("learning_rate", 0.01)
        app_state.epochs = hyperparams.get("epochs", 1000)
        app_state.lambda_l2 = hyperparams.get("lambda_l2", 0.0)
        app_state.threshold = hyperparams.get("threshold", 0.5)
        ...
        app_state.model = LogisticRegressionGD()
    ...
    def validate_session(self, session_data: SessionData) -> bool:
        ...
        if "threshold" not in hyperparams:
            raise ValueError("!Missing threshold hyperparameter!")
        return True
