...
def select_features_and_target_for_classification(dataset: Dataset) -> Prepareddata:
    """
    Interactive feature and target selection for classification.
    
    Uses the universal select_features_and_target from base_data,
    then validates that the target is binary 0/1.
    
    Args:
        dataset: Loaded dataset
    
    Returns:
        Prepareddata with selected X and y (binary target)
    """
    ...
    if len(unique_values) == 2:
        if set(unique_values) != {0, 1}:
            ...
            prepared.Y = (prepared.Y == unique_values[1]).astype(int)
    else:
...
def select_features_interactive(s: AppState) -> None:
    if s.dataset is None:
        ...
        return
    try:
        s.prepareddata = select_features_and_target_for_classification(s.dataset)
        rebuild_split(s)
