...
def select_features_and_target_multiclass(dataset: Dataset) -> Tuple[Prepareddata, List[str]]:
    """
    ...
    Returns:
        Tuple of (Prepareddata with selected X and y, list of original class names)
    """
    ...
    # Save original class names BEFORE remapping
    original_class_names = [str(v) for v in sorted(unique_values)]
    ...
    return prepared, original_class_names
...
def detect_class_names(s: MultinomialAppState,
                       original_names: Optional[List[str]] = None) -> None:
    """
    Auto-detect and optionally rename class names based on dataset.
    
    If original_names are provided (from CSV labels), they are used as defaults.
    Otherwise falls back to "Class 0", "Class 1", etc.
    """
    if s.prepareddata is None:
        return
    
    if original_names:
        s.class_names = list(original_names)
    else:
        unique_vals = np.unique(s.prepareddata.Y)
        s.class_names = [f"Class {int(v)}" for v in sorted(unique_vals)]
    
    # Ask user if they want to keep these names
    names_str = ", ".join(s.class_names)
    if ask_yes_no(f"Use class names ({names_str})?", default=True):
        return
    
    print("\nEnter custom names for each class:")
    for k in range(len(s.class_names)):
        new_name = input(f"  {s.class_names[k]}: ").strip()
        if new_name:
            s.class_names[k] = new_name
...
def select_features_interactive_multinomial(s: MultinomialAppState) -> None:
    ...
    if s.dataset is None:
        ...
        return
    try:
        s.prepareddata, orig_names = select_features_and_target_multiclass(s.dataset)
        rebuild_split(s)
        detect_class_names(s, orig_names)
    ...
    while True:
        ...
        if choice == 0:
            ...
            if s.model is None or not s.model.is_trained:
                ...
                continue
            if s.prepareddata is None:
                print("✗ No features/target selected!")
                pause()
                continue

            csv_path = input("\nEnter CSV path with feature columns: ").strip()
            if not csv_path:
                print("✗ Invalid path!")
                pause()
                continue
            ...
            output_csv = input(f"Output CSV path [{default_output}]: ").strip()
            if not output_csv:
